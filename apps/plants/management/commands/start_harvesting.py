...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--days', type=int, default=7, help='Number of days to run the harvester')
        parser.add_argument('--rate', type=int, default=5, help='Plants per hour to harvest')
    ...
    def handle(self, *args, **options):
        ...
        if not api_key:
            self.stdout.write(self.style.ERROR('OPENAI_API_KEY environment variable is missing.'))
            return
        ...
        def insert_harvested_plants(harvested_data_list):
            self.stdout.write(f"Inserting {len(harvested_data_list)} harvested plants into DB...")
            ...
            self.stdout.write(self.style.SUCCESS(f"Successfully inserted {len(harvested_data_list)} plants."))

        try:
            self.stdout.write(f"Starting AI Plant Harvest Background Task ({days} days, {rate} plants/hr)...")
            ...
            self.stdout.write(self.style.SUCCESS('Background task started successfully. Keep this terminal open to continue harvesting.'))
            ...
            self.stdout.write(self.style.ERROR(f"Could not start AI plant background harvester: {e}"))
